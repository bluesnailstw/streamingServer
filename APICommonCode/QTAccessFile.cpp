...
Bool16 QTAccessFile::AccessAllowedAdaptedRedis  (QTSS_StandardRTSP_Params* inParams, char *userName, char**groupArray, UInt32 numGroups, QTSS_ActionFlags inFlags, Bool16 *outAllowAnyUserPtr, void *extraDataPtr)
{
    ...
    *outAllowAnyUserPtr = false;
    ...
    char remoteURL[100] = {0};
    StrPtrLen theClientTokenID(remoteURL,sizeof(remoteURL));
    QTSS_GetValue(inParams->inClientSession,qtssCliSesPresentationURL, 0, (void*)theClientTokenID.Ptr, &theClientTokenID.Len);
    qtss_printf("theClientTokenID: %s\n", theClientTokenID.Ptr);

    if(haveUserName){
        qtss_printf("haveUserName: %s\n", userName);
        result = true;
    }else{
        result = false;
    }
    //todo
//    if (haveGroups){
//        if(inFlags == qtssActionFlagsWrite){
//            StrPtrLen pushGroup("pushuser");
//            result = TestGroup(&pushGroup, userName, groupArray, numGroups, extraDataPtr);
//            qtss_printf("INFO qtssActionFlagsWrite TestGroup is:%d\n", result);
//        }else if(inFlags == qtssActionFlagsRead){
//            StrPtrLen pullGroup("pulluser");
//            result = TestGroup(&pullGroup, userName, groupArray, numGroups, extraDataPtr);
//            qtss_printf("INFO qtssActionFlagsRead TestGroup is:%d\n", result);
//        }else{
//            result = false;
//            qtss_printf("ERROR inFlags is:%d\n", inFlags);
//        }
//    }else{
//        qtss_printf("ERROR haveGroups is:%d\n", haveGroups);
//        result = false;
//    }
...
QTSS_Error QTAccessFile::AuthorizeRequestAdaptedRedis(QTSS_StandardRTSP_Params *inParams, Bool16 allowNoAccessFiles,
                                                      QTSS_ActionFlags noAction, QTSS_ActionFlags authorizeAction,
                                                      Bool16 *outAuthorizedPtr, Bool16 *outAllowAnyUserPtr)
{
    ...
    *outAuthorizedPtr = false;
    ...
    Bool16 allowRequest = this->AccessAllowedAdaptedRedis(inParams, username, groupCharPtrArray, numGroups, authorizeAction, outAllowAnyUserPtr);
    qtss_printf("allowRequest=%d\n", allowRequest);

...
//        }
    ...
    //todo
    QTSS_AuthScheme authScheme = qtssAuthDigest;
    theErr = QTSS_SetValue(theRTSPRequest, qtssRTSPReqAuthScheme, 0, (void*)&authScheme, sizeof(authScheme));
    if (theErr != QTSS_NoErr)
        return theErr;
...
//    (void)QTSS_SetValue(theUserProfile, qtssUserRealm, 0, (void*)(authRealm->Ptr), (authRealm->Len));
    ...
    //todo mark
    char * authRealmStr = "Qhome streaming server";
    (void)QTSS_SetValue(theUserProfile, qtssUserRealm, 0, (void*)authRealmStr, strlen(authRealmStr));
    qtss_printf("qtssUserRealm:%s\n", authRealmStr);
...
//        (void)QTSS_SetValue(theUserProfile, qtssUserPassword, 0, (void*)((profile->digestPassword).Ptr), (profile->digestPassword).Len);
        //todo waiting for redis
    char* userStr = "952d6b9fc7a38224b58eb4f45502c9da";
    char* PasswordStr = "1611af26bdab81bb4c0f230a9a959703";

    //char* digestPasswordStr = "47242963bc82322b532ccc3220b3efbd";
    char* digestPasswordStr = rtspDigest(userStr, PasswordStr, authRealmStr);
    qtss_printf("digestPasswordStr:%s\n", digestPasswordStr);
    (void)QTSS_SetValue(theUserProfile, qtssUserPassword, 0, (void*)digestPasswordStr, strlen(digestPasswordStr));
//    }
...
//        }
//    }
//    char* PushGroupStr = "pushuser";
//    char* PullGroupStr = "pulluser";
//
//    (void)QTSS_SetValue(theUserProfile, qtssUserGroups, 0, (void*)PushGroupStr, strlen(PushGroupStr));
//    (void)QTSS_SetValue(theUserProfile, qtssUserGroups, 1, (void*)PullGroupStr, strlen(PullGroupStr));
...
//    return  accessFile.AuthorizeRequest(inParams,allowNoAccessFiles, noAction, authorizeAction, &authorized, &allowAnyUser);
    accessFile.AuthorizeRequestAdaptedRedis(inParams,allowNoAccessFiles, noAction, authorizeAction, &authorized, &allowAnyUser);
    if( (authorized == false) && (authorizeAction & qtssActionFlagsRead) ) //handle it
    {
